...
result_names = ["results/", "results_reversed/"]
...
plt.pcolormesh(CHIs_matrix, JSs_matrix, enrichment_matrix, cmap="nipy_spectral", shading="auto", alpha=0.5)
ax = plt.gca()
...
cbar = plt.colorbar()
[i.set_linewidth(1.5) for i in cbar.ax.spines.values()]
...
plt.contour(CHIs_matrix, JSs_matrix, enrichment_matrix, levels=[0.05], colors="black", linewidths=3, linestyles="--")
plt.text(2.045, 0.2, "0.05", fontsize=18, color="black")
...
for (js, chi, interfaces) in zip(orig_JSs_matrix[:], orig_CHIs_matrix[:], interfaces_matrix[:])
    plt.scatter([chi], [js], color=colors_dict[Int(interfaces)], s=60, edgecolors="black", linewidths=0.75)
end
