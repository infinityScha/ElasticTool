...
const MAIN_PATH = "results_fixed/$JS_CURR/$CHI_CURR/"
...
UNFIXED_PATH = "results/$JS_CURR/$CHI_CURR/"
...
if false #isfile(MAIN_PATH*"states/final.coords")
    ...
    # if already ran this script for the previous hamiltonian, then copy the previous state to the current state folder as the initial
    if isfile(UNFIXED_PATH*"states/final.coords")
        cp(UNFIXED_PATH*"states/final.coords", MAIN_PATH*"initial.coords", force=true)
        cp(UNFIXED_PATH*"states/final.vol_patches", MAIN_PATH*"initial.vol_patches", force=true)
        cp(UNFIXED_PATH*"states/final.rem_patches", MAIN_PATH*"initial.rem_patches", force=true)
    else
        # copy the previous state to the current state folder
        PREV_PATH = "results_fixed/$JS_PREV/$CHI_PREV/"
        ...
        # if previous is "basic_structure" then change the water volume to 0.9/0.95 of the previous water volume
        if JS_PREV == "basic_structure"
            v_old = parse(Float64, get_patch_property(MAIN_PATH * "initial.vol_patches", 3, "V0"))
            change_patch_property(MAIN_PATH * "initial.vol_patches", 3, "V0", v_old*0.9/0.95)
        end
...
lines = readlines("enrichment.csv")
