...
if isfile(MAIN_PATH*"states/final.coords")
    ...
    change_patch_property(MAIN_PATH*"initial.vol_patches", 1, "V0", vol_patches_open[1].v₀)
    change_patch_property(MAIN_PATH*"initial.vol_patches", 2, "V0", vol_patches_open[2].v₀)
    change_patch_property(MAIN_PATH*"initial.vol_patches", 3, "V0", vol_patches_open[3].v₀)
    # change thicknesses to 0.5 nm
    change_patch_property(MAIN_PATH*"initial.vol_patches", 1, "H0", 0.5)
    change_patch_property(MAIN_PATH*"initial.vol_patches", 2, "H0", 0.5)
    change_patch_property(MAIN_PATH*"initial.vol_patches", 3, "H0", 0.5)
    ...
    # run the minimization
    which_to_adjust = [1,1,1,1,1,1]
    when_to_adjust = [5,500,1000,2000,3000,4000]
    adjust_resolution_to = [1.5, 1.2,1.0,0.8,0.65,0.5]
    coords, indep_coords, vol_patches, rem_patches = minimize(coords, indep_coords, vol_patches, rem_patches, which_to_adjust, when_to_adjust, adjust_resolution_to, μᵥ=5.0e4, μᵩ=1.0e4, remesh_spacing=1, mult_val=0.999, final_ΔEₘₐₓ=2.5e-4, initial_ΔEₘₐₓ=1.0)
end
