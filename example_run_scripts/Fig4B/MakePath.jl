PREV_CHI, CURR_CHI = ARGS[1], ARGS[2]


const MAIN_PATH = "results/$CURR_CHI/"
...
const CHI = parse(Float64, CURR_CHI)
...
if isdir(MAIN_PATH * "final_path")
    # load the final path
    coords_arr, indep_coords_arr, vol_patches_arr, rem_patches_arr = load_string(MAIN_PATH * "final_path/", M)
    # relax it for 150 steps
    coords_arr, indep_coords_arr, vol_patches_arr, rem_patches_arr = neb_method(coords_arr, indep_coords_arr, vol_patches_arr, rem_patches_arr, neb_coords_place_arr, initial_step_num = 0, initial_ΔEₘₐₓ=2.0e-5, final_ΔEₘₐₓ=2.0e-5, remesh_spacing=1, μᵥ=1.0e4, μᵩ=1.0e4, mult_val=0.999, max_steps=150)
else
    # copy the path found for mixed
    coords_arr, indep_coords_arr, vol_patches_arr, rem_patches_arr = load_string("results/$PREV_CHI/final_path/", M)
